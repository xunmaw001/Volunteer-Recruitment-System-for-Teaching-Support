    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(zhaopin !=null){
            ...
            //级联表 支援学校
            //级联表
            XuexiaoEntity xuexiao = xuexiaoService.selectById(zhaopin.getXuexiaoId());
            if(xuexiao != null){
            BeanUtils.copyProperties( xuexiao , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "xuexiaoId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setXuexiaoId(xuexiao.getId());
            }
            //修改对应字典表字段
    ...
    */
    ...
    public R save(@RequestBody ZhaopinEntity zhaopin, HttpServletRequest request){
        ...
        if(false)
            ...
            zhaopin.setXuexiaoId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<ZhaopinEntity> queryWrapper = new EntityWrapper<ZhaopinEntity>()
            .eq("xuexiao_id", zhaopin.getXuexiaoId())
...
//                            zhaopinEntity.setXuexiaoId(Integer.valueOf(data.get(0)));   //公司 要改的
...
//                            zhaopinEntity.setZhaopinAddress(data.get(0));                    //学校地点 要改的
        ...
        ZhaopinEntity zhaopin = zhaopinService.selectById(id);
            if(zhaopin !=null){
                ...
                //级联表
                    XuexiaoEntity xuexiao = xuexiaoService.selectById(zhaopin.getXuexiaoId());
                if(xuexiao != null){
                    BeanUtils.copyProperties( xuexiao , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setXuexiaoId(xuexiao.getId());
                }
                //修改对应字典表字段
                dictionaryService.dictionaryConvert(view, request);
